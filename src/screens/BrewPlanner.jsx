  ...
  const handleSubmit = async (e, index) => {
    ...
    try {
      ...
      const filteredTemplates = allTemplates.filter(template => {
        ...
        return ["brewday", "fermentationstart", "fermentationend", "serving"].includes(anchor);
      });
      ...
      for (const template of filteredTemplates) {
        ...
        const scheduledDate = new Date(anchorDate.getTime() + offset * 86400000);

        await addDoc(collection(db, "userPlans", docRef.id, "tasks"), {
          taskTemplateId: template.id,
          taskName: template.taskName || "",
          anchorEvent: template.anchorEvent || "",
          autoGeneratedBy: template.autoGeneratedBy || "",
          canOverlap: template.canOverlap === "Yes" || template.canOverlap === true,
          dayOffset: offset,
          scheduledDate,
          defaultAssignmentPool: template.defaultAssignmentPool || "",
          estimatedDurationMin: Number(template.estimatedDurationMin) || 0,
          fallbackRoles: template.fallbackRoles || "",
          isAutomated: template.isAutomated === true,
          isCritical: template.isCritical === true,
