...
// 🔧 1. Mirror final data into userPlans
const planRef = doc(db, "userPlans", id);
await setDoc(
  planRef,
  {
    finalOG: finalOGValue,
    finalVolume: finalVolumeValue,
    status: "inFerment",
  },
  { merge: true }
);

// 🔧 2. Inject post-brew tasks
const templatesSnap = await getDocs(
  query(
    collection(db, "taskTemplates"),
    where("anchorEvent", "in", ["fermentationend", "serving"])
  )
);
...
const baseDate = new Date(plan.startDate);
const planDuration = fermentationDays[plan.fermentationType] || 14;

for (const docSnap of templatesSnap.docs) {
  const template = docSnap.data();
  const offset = parseInt(template.dayOffset || "0", 10);
  const scheduledDate = new Date(baseDate);
  scheduledDate.setDate(scheduledDate.getDate() + planDuration + offset);

  const taskData = {
    taskTemplateId: docSnap.id,
    taskName: template.taskName || "",
    anchorEvent: template.anchorEvent || "",
    autoGeneratedBy: template.autoGeneratedBy || "Post-Brew Trigger",
  ...
  await addDoc(collection(db, `userPlans/${id}/tasks`), taskData);
}

console.log("✅ Post-brew tasks injected successfully.");
